...
def main_entity(configuration_file_path):
    ...
    try:
        response = s3.list_objects_v2(Bucket=bucket_name, Prefix=input_file)
        ...
        print(f"An error occurred while listing objects in the S3 bucket: {e}")
    ...
    try:
        for key in dict_map['Nested']:
            ...
            try:
                ...
                csv_buffer = StringIO()
                df.to_csv(csv_buffer, sep=",", escapechar="\\", quoting=csv.QUOTE_ALL, index=None, na_rep='', header=True)
